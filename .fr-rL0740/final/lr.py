...
def prepare_data(images):
    ...
    for i, image_file in enumerate(images):
        ...
        if 'no' in image_file.lower() or 'no' in image_file.lower():
            y[0, i] = 1
        elif 'yes' in image_file.lower():
            y[0, i] = 0
    return X, y
...
d = model(train_set_x, train_set_y, test_set_x, test_set_y, num_iterations = 1, learning_rate = 0.003, print_cost = True)

test_image = "no4.jpeg"
my_image = read_image(test_image).reshape(1, ROWS*COLS*CHANNELS).T
my_predicted_image = predict(d["w"], d["b"], my_image)
print(np.squeeze(my_predicted_image))
if np.squeeze(my_predicted_image)==0.0:
    print("corona Detected")
else:
    print("No corona Detected")

import joblib
joblib.dump(d, 'model_joblib.h5')
